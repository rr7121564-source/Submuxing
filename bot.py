...
async def extract_subtitles(mkv_path, original_name):
    ...
    for stream in data.get('streams', []):
        ...
        ext_proc = await asyncio.create_subprocess_exec(
            *ext_cmd, stderr=asyncio.subprocess.DEVNULL
        )
        await ext_proc.wait()
...
async def cmd_sub(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg.reply_to_message or not msg.reply_to_message.document:
        await msg.reply_text("Please reply to an MKV file message with /sub.")
        return
...
async def cmd_extract(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.message
    if not msg.reply_to_message or not msg.reply_to_message.document:
        await msg.reply_text("Please reply to an MKV file message with /extract.")
        return
    ...
    try:
        ...
        if not extracted_files:
            await status_msg.edit_text("No subtitle streams found in this MKV. [X]")
            return
