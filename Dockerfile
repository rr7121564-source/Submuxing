import os, time, asyncio, threading, sqlite3
...
from database import init_db, is_user_auth, is_chat_auth, add_processed_id, DB_PATH
from utils import mux_video, clean_temp_files
...
async def start_task(update, context, final_name):
    ...
    data = {'chat_id': update.effective_chat.id, 'mkv_id': context.user_data['mkv_id'], 
            'sub_id': context.user_data['sub_id'], 'name': final_name, 'thumb': context.user_data.get('thumb_id')}
