...
async def mux_video(mkv_path, sub_path, output_path, chat_id, status_msg):
    ...
    for idx, f in enumerate(os.listdir("fonts")):
        ...
        font_args.extend(["-attach", fp, f"-metadata:s:t:{idx}", f"mimetype=application/x-truetype-font"])
        ...
        if line.startswith('out_time_us='):
            try:
                ...
                if duration > 0 and time.time() - last_up > 12:
                    ...
                    await status_msg.edit_text(f"⚙️ Muxing: {perc:.2f}%", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Cancel", callback_data=f"cancel_{chat_id}")]]))
                    last_up = time.time()
            except: pass
    await proc.wait()
